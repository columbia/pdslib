use crate::budget::traits::{Filter, FilterStorage, FilterResult};
    ...
    K: Eq + std::hash::Hash
{
    ...
    fn try_consume(&mut self, filter_id: K, budget: Budget) -> Result<FilterResult, ()> {
...
mod tests {
    use super::*;
    use crate::budget::pure_dp_filter::{PureDPBudgetFilter, PureDPBudget};

    #[test]
    fn test_hash_map_filter_storage() {
        let mut storage: HashMapFilterStorage::<usize, PureDPBudgetFilter, PureDPBudget> = HashMapFilterStorage::new();
        storage.new_filter(1, PureDPBudget { epsilon: 1.0 }).unwrap();
        assert!(storage.try_consume(1, PureDPBudget { epsilon: 0.5 }).unwrap().is_ok());
        assert!(storage.try_consume(1, PureDPBudget { epsilon: 0.6 }).unwrap().is_err());

        // Filter 2 does not exist
        assert!(storage.try_consume(3, PureDPBudget { epsilon: 0.2 }).is_err());
